    ...
    def __init__(self):
        ...
        self.dists = {
            'F(x) = x^4': {
                'x_min': 0.0,
                'x_max': 1.0,
                'pdf': lambda x: 4 * x**3,          # Похідна від F(x)
                'cdf': lambda x: x**4,              # Функція розподілу
                'inv_cdf': lambda g: g**0.25        # Обернена функція (x = F^(-1)(gamma))
            },
            'F(x) = 2*sin(x)': {
                'x_min': 0.0,
                'x_max': np.pi / 6,
                'pdf': lambda x: 2 * np.cos(x),     # Похідна від F(x)
                'cdf': lambda x: 2 * np.sin(x),     # Функція розподілу
                'inv_cdf': lambda g: np.arcsin(g / 2) # Обернена функція
            }
        }
        
        self.current_name = 'F(x) = x^4'
