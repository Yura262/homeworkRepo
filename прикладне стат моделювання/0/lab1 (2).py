...
class DistributionSimulator:
    ...
    def draw_theoretical(self):
        ...
        if self.current_dist == 'Poisson':
            ...
            pmf = self.binom_pmf
            cdf = self.binom_cdf
            title = f'Біноміальний розподіл (n=222, p≈0.511)'
            self.ax_dist.set_xlim(80, 150) # Показуємо лише ту зону, де є реальна ймовірність
            
            # Інтервалів 222, тому замість суцільного розмальовування ставимо тонкі межі
            self.ax_line.axvspan(0, 1, alpha=0.1, color='blue')
            # Малюємо вертикальні лінії меж тільки для значень k від 90 до 135
            for p in cdf[90:135]:
